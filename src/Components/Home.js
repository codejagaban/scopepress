...
const Home = () => {
	const content = [
		{ image: '/images/bg2.webp', title: 'AWESOME ACADEMIC PROOFREADING & EDITING', break: 'EXPERTS.', description: 'Improve your paper with language editing and proofreading from our subject experts.' },
		{ image: '/images/bg3.webp', title: 'WE HELP YOU ACHIEVE YOUR ACADEMIC', break: 'GOALS.', description: 'Get your papers edited and published swiftly', button: 'Get Started', url: '/quote' },
		{ image: '/images/bg1.webp', title: 'WE HELP YOU ACHIEVE YOUR ACADEMIC', break: 'GOALS.', description: 'On time delivery', button: 'Get Started', url: '/quote' },
					...
					<div className="">
						<Slider autoplay={2000} infinite>
							{content.map((item, index) => (
								<div
									key={index}
									style={{ background: ` linear-gradient(rgba(9, 14, 59, 0.8), rgba(9, 14, 59, 0.5)),url('${item.image}') no-repeat`, backgroundSize: 'cover', backgroundPosition: 'center', height: '75vh' }}
								>
									<div className="container">
										<div className="slider-text">
											<h1>{item.title} <span className="text-danger">{item.break}</span></h1>
											<p className="">{item.description}</p>
											{item.button && <p className="btn mt-2 btn-brand text-white px-6 py-2 rounded rounded-3">
												<Link to={item.url} role="button" className="font-weight-medium text-small pr-2 pl text-white ">{item.button}</Link>
											</p>}
										</div>
									</div>
								</div>
